...
var matchHashMap = {};
dataNow = DateTime.local().setLocale('it-IT');
var replace = true;
var keepLive = true;
var todayDay = DateTime.local().setLocale('it-IT').day;
var matchTimeStart = undefined;
var matchTimeEnd = undefined;
var countLive = 0;
var newMargingSelected = false;

var latest = DateTime.local().setLocale('it-IT');
fetchRangeTime()

cron.schedule('*/10 * * * * *', () => {
    if (DateTime.local().setLocale('it-IT') > matchTimeStart && keepLive) {
        newMargingSelected = false;
        updateLive(0).then(() => {
            if (Object.keys(matchHashMap).length !== 0) {
                latest = DateTime.local().setLocale('it-IT');
        ...
        if (newMargingSelected == false) {
            if (DateTime.local().setLocale('it-IT').day !== todayDay) {
                setRangeTime(0);
                updateTimestampLastCall();
            } else {
                setRangeTime(1);
...
async function updateTimestampLastCall() {
    ...
    let dataTime = {
        log: DateTime.local().setLocale('it-IT')
    }
...
async function setRangeTime(dayPlus) {
    ...
    var orarioInizio = DateTime.fromFormat(`${DateTime.local().setLocale('it-IT').plus({ days: dayPlus }).toFormat('yyyy-MM-dd')} 23:59:00`, formatoInput);
    var orarioFine = DateTime.fromFormat(`${DateTime.local().setLocale('it-IT').plus({ days: dayPlus }).toFormat('yyyy-MM-dd')} 00:00:00`, formatoInput);
        ...
        (async () => {
            ...
            axios.put(url, jsonData, config)
                .then(response => {
                    ...
                    const updMatchUrl = `http://${process.env.APIBETGRAM}/api/insert/match/${DateTime.local().setLocale('it-IT').minus({ days: dayCheck }).toFormat('yyyy-MM-dd')}`;
